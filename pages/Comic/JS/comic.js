...
$(document).ready(function () {
    var i = 0;
    var imgWidth = $(".comic_slider .comic_content").children("li").width();
    var clone = $(".comic_slider .comic_content li").first().clone();
    $(".comic_slider .comic_content").append(clone);
    for (var j=0;j<$(".comic_slider .comic_content").children("li").length-1;j++){
        $(".comic_slider .comic_slide_hover").append("<li>");
    }
    $(".comic_slider .comic_slide_hover li").first().addClass("on");
    //-------
    //图片向左移动
    //-------
    function moveLeft(){
        ...
        var size = $(".comic_slider .comic_content").children("li").length;
        // console.log(size);

        if (i == size){
            $(".comic_slider .comic_content").css({left:0});
            i=1;
        }

        $(".comic_slider .comic_content").stop().animate({
            left:-imgWidth*i
        },500);

        if (i == size-1){
            $(".comic_slider .comic_slide_hover li").eq(0).addClass("on").siblings().removeClass("on");
        }else {
            $(".comic_slider .comic_slide_hover li").eq(i).addClass("on").siblings().removeClass("on");
        }
    }
...
//    -------
    function moverRight() {
        ...
        if (i == -1){
            $(".comic_slider .comic_content").css({left:-(size-1)*imgWidth});
            i=size-2;
        }
...
//    鼠标点击
//    ----------
    $(".comic_slider .comic_slide_hover li").click(function () {
...
//    鼠标移入
    $(".comic_slider .comic_slide_hover").hover(function () {
        clearInterval(time);
    },function () {
        time = setInterval(moveLeft,3000);
    });
    $(".comic_slider .btn").hover(function () {
        clearInterval(time);
    },function () {
        time = setInterval(moveLeft,3000);
    });
    ...
    var comicData = {
                "comic":[
                        {"src":"1.jpg","txt1":"What If? The Punisher #1","txt2":"Potts, Ramirez","score":"3"},
                        {"src":"2.jpg","txt1":"Hello","txt2":"Potts, Ramirez","score":"4"},
                        {"src":"3.jpg","txt1":"What If? The Punisher #1","txt2":"Potts, Ramirez","score":"3"},
                        {"src":"4.jpg","txt1":"What If? The Punisher #1","txt2":"Potts, Ramirez","score":"5"},
                        {"src":"5.jpg","txt1":"What If? The Punisher #1","txt2":"Potts, Ramirez","score":"4"},
                        {"src":"6.jpg","txt1":"What If? The Punisher #1","txt2":"Potts, Ramirez","score":"3"},
                        {"src":"7.jpg","txt1":"What If? The Punisher #1","txt2":"Potts, Ramirez","score":"2"},
                        {"src":"8.jpg","txt1":"What If? The Punisher #1","txt2":"Potts, Ramirez","score":"3"}]
...
//     console.log(comicData);
    ...
    $(".read_more").click(function () {
        // console.log(comicData);
        $.each(comicData.comic,function (index,value) {
            console.log(value.txt1);
            var box = $("<li>").appendTo(".comic_list");
            var a = $("<a>").appendTo(box);
            $("<img>").addClass("shadow").attr("src","images/"+$(value).attr("src")).appendTo(a);
            $("<span>").text(value.txt1).appendTo(box);
            $("<span>").text(value.txt2).appendTo(box);
            // console.log($(this));
            // console.log(value.score);
            var comic_score = $("<div>").addClass("comic_score").appendTo(box);
            var sul = $("<ul>").appendTo(comic_score);
            for (var i = 0;i<value.score;i++){
                var sLi = $("<li>").appendTo(sul);
                var sa = $("<a>").appendTo(sLi);
                $("<i>").addClass("fa fa-star").appendTo(sa);
            }
            for (var j=0;j<5-value.score;j++){
                var sLi = $("<li>").appendTo(sul);
                var sa = $("<a>").appendTo(sLi);
                $("<i>").addClass("fa fa-star-o").appendTo(sa);
            }
        })
    });
...
//    到头部
    $(window).scroll(function () {
        ...
        if (height>400){
            $(".goTo_top").stop().fadeIn();
        } else {
            $(".goTo_top").stop().fadeOut();
        }
    });
//    点击回头
    $(".goTo_top").stop().click(function () {
        $("body,html").animate({
            scrollTop:0
        },500);
    });
